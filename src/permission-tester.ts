...
async function testSinglePermission(
	...
	try {
		...
		if (test.method !== "GET" && response.status >= 400 && response.status < 403) {
			return {
				test,
				status: "pass",
...
export async function testTokenPermissions(
	...
	const requiredFailed = requiredTests.filter((r) => r.status !== "pass").length;
	const optionalPassed = optionalTests.filter((r) => r.status === "pass").length;
	const optionalFailed = optionalTests.filter((r) => r.status !== "pass").length;
	...
	if (optionalFailed > 0) {
		...
		// Group by category
		const categorizedOptional = failedOptional.reduce((acc, test) => {
...
export function formatPermissionReport(report: PermissionReport): string {
	const { overallStatus, requiredPassed, requiredFailed, optionalPassed, optionalFailed, recommendations } = report;
